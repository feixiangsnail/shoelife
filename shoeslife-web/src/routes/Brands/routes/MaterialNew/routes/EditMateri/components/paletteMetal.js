...
PanelModel.prototype.init = function(param) {
    ...
    this.camera = camera;
    var type=param.type;
    this.addModel(param.url, param.materialParams,type);
    this.run();
}
PanelModel.prototype.addModel = function(url, materialParams,type) {
    ...
    loaderCTM.loadParts(url, function(geometries) {
        for (var i = 0; i < geometries.length; i++) {
            ...
            if(i==geometries.length-1){
                 that.setMaterial(materialParams,type);
            }
        }
    ...
    if (materialParams) {
        ...
        if (type) {
            material = new THREE.MeshStandardMaterial({
                color: new THREE.Color(materialParams.color) || 0x998722,
            ...
            material = new THREE.MeshPhongMaterial({
                color: new THREE.Color(materialParams.color) || 0x00ff00,
                specular: new THREE.Color(materialParams.specular) || 0x000000,
                shininess: materialParams.shininess || 30,
                transparent: materialParams.transparent || true,
                opacity: materialParams.opacity || 1.0,
                side: THREE.DoubleSide,
            });
